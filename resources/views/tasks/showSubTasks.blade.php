        ...
        <div class="mb-10 py-3 ml-16 leading-normal flex text-green-500 rounded-lg" role="alert">
            ...
            @if (auth()->user()->role == 'Administrator')
                <div style="margin-left: 900px">
                    <form action="{{ route('create.subtask') }}">
                    ...
                    <tbody>
                        @foreach ($tasks as $task)
                            @foreach ($task->SubTasks as $subtasks)                    
                                <tr>                                    
                                    <td align="center">{{ $subtasks->title}}</td>
                                    <td align="center">@if($subtasks->isCompleted) 'Completed' @else 'Pending' @endif</td>
                                    <td>
                                        {{$task->title}}
                                    </td>
                                    <td align="center">
                                        <div class="container justify-between inline-flex">
                                            @if ($subtasks->isCompleted == false)                                                
                                                <div style="width: 33%">
                                                    <form action="{{ route('complete.subtask',$subtasks->id) }}" method="POST">
                                                        @csrf
                                            ...
                                            @endif
                                            <div style="width: 33%">
                                                <form action="{{ route('edit.subtask',$subtasks->id) }}">
                                                    @csrf
                                            ...
                                            <div style="width: 34%">
                                                <form action="{{ route('delete.subtask',$subtasks->id) }}" method="POST">
                                                    @method('DELETE')
                                                    @csrf
